...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // After the OAuth redirect, Supabase drops us back on this page with a
      // session in the URL hash. The AuthProvider's onAuthStateChange listener
      // picks up SIGNED_IN and the user is then sent to their dashboard by the
      // logged-in-redirect logic elsewhere. If Google OAuth isn't configured
      // in Supabase's dashboard, this call returns an error we surface below.
      ...
      setError(
        msg.toLowerCase().includes("provider")
          ? "Google sign-in isn't enabled on this project yet. Use email + password for now."
          : msg
      );
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              ...
              className="w-full bg-white border border-gray-300 text-gray-700 py-3 px-4 rounded-lg font-medium hover:bg-gray-50 transition-colors flex items-center justify-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {oauthLoading ? (
